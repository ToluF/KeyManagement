 ...
 */
const KeySelectionStep = ({ keys = [], selectedKeys = [], onSelect, onBack, onNext, query, onSearchChange }) => {
  // const [searchQuery, setSearchQuery] = useState('');
 
  // Filter keys based on search query
  const filteredKeys = keys.filter(
    (key) => 
      key.keyCode.toLowerCase().includes(query.toLowerCase()) ||
      key.location.toLowerCase().includes(query.toLowerCase())
